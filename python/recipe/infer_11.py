...
def text_recognition(queue):
    ...
    while True:
        if not queue.empty():
            ...
            if current_time - last_rec_time >= 2:
                # 每2秒进行一次文字识别
...
if __name__ == '__main__':
    queue = multiprocessing.Queue(maxsize=5)  # 设置一个队列，用于存放视频帧

    video_process = multiprocessing.Process(target=video_capture, args=(queue,))
    text_process = multiprocessing.Process(target=text_recognition, args=(queue,))
